export class Projectile {
  constructor(
    ...
    explosionRadius = 0,
    explosionDamage = 0,
    minDetonationRange = 0,
    maxDetonationRange = 0,
    shieldDisruption = false,
    disruptionDuration = 0,
    armingTime = 0,
    lifetime = 0,
    asteroidDamageMultiplier = 1.0,
  ...
  update(deltaTime) {
    ...
    const distanceMoved = Math.sqrt(
      (this.velocity.x * deltaTime) ** 2 + (this.velocity.y * deltaTime) ** 2,
    );
    ...
    if (this.trail && Math.random() > 0.6) {
      // Only add particles sometimes for performance
    ...
    if (
      ...
      this.explosive &&
      !this.hasExploded &&
      this.minDetonationRange > 0 &&
      this.maxDetonationRange > 0
    ) {
      ...
      if (this.distanceTraveled >= this.minDetonationRange) {
        ...
        const rangeProgress =
          (this.distanceTraveled - this.minDetonationRange) /
        ...
        if (
          Math.random() < detonationChance ||
          this.distanceTraveled >= this.maxDetonationRange
        ) {
  ...
  createSplashEffect(world, soundManager) {
    ...
    if (
      ...
      Object.values(window.game.multiplayer.players).forEach((player) => {
        ...
        if (
          typeof world.isInSafeZone === 'function' &&
          world.isInSafeZone(player)
        )
          return;
    ...
    if (world.asteroids) {
      world.asteroids.forEach((asteroid) => {
        ...
        if (distance <= this.explosionRadius + asteroid.radius) {
          ...
          const damageFalloff = Math.max(
            0,
            1 - distance / this.explosionRadius,
          );
