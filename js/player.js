...
export class Player {
  ...
  handlePlayerCollision(otherPlayer, soundManager) {
    ...
    if (
      ...
      console.warn(
        'handlePlayerCollision called with invalid otherPlayer:',
        otherPlayer,
      );
      return;
    }
    ...
    if (this.collisionCooldown <= 0) {
      ...
      // Calculate relative velocity
      const relVelX =
        this.velocity.x - (otherPlayer.velocity ? otherPlayer.velocity.x : 0);
      const relVelY =
        this.velocity.y - (otherPlayer.velocity ? otherPlayer.velocity.y : 0);

      // Calculate impulse (how strongly we bounce)
      const baseImpulse = Math.sqrt(relVelX * relVelX + relVelY * relVelY);
      const impulseStrength = Math.max(
        90,
        (1 + this.bounceStrength) * baseImpulse * 0.3,
      );
      ...
      const impactForce = Math.sqrt(relVelX * relVelX + relVelY * relVelY);
      if (impactForce > 200) {
        // Only deal damage for high-speed collisions
  ...
  // Snappier tuning: increase accel and top speed, reduce damping for responsiveness
  this.rotationAccel = 9.0; // angular acceleration factor (higher = faster spin-up)
  this.rotationDamping = 6.0; // damping factor to reduce angular velocity when no input (lower = less braking)
  this.precisionRotationMultiplier = 0.6; // multiplier while firing for finer control (higher = more responsive while firing)
  this.maxRotationSpeed = 2.4; // rad/s maximum angular velocity (safety cap) - increased for snappier turns
  this.precisionMaxRotationMultiplier = 0.6; // when precision active, reduce the max rotation (but still responsive)
  ...
  update(deltaTime, input, soundManager) {
    ...
    if (
      ...
      input.keys.includes('KeyW') ||
      (input.thrustAmount && input.thrustAmount > 0)
    ) {
    ...
    if (this.afterburnerActive) {
      ...
      this.energy = Math.max(
        0,
        this.energy - this.afterburnerEnergyDrain * deltaTime,
      );
    ...
    if (
      ...
      window.shopSystem &&
      window.shopSystem.availableWeapons
    ) {
      weapon = window.shopSystem.availableWeapons.find(
        (w) => w.id === this.currentWeaponId,
      );
    }
    ...
    let baseCooldown =
      weapon && weapon.stats ? weapon.stats.cooldown : this.fireCooldownTime;
    ...
    if (this.fireRateBoosts && this.fireRateBoosts.length > 0) {
      ...
      fireRateMultiplier = this.fireRateBoosts.reduce(
        (min, b) => (b.multiplier < min ? b.multiplier : min),
        1,
      );
    }
    ...
    if (
      this.energy <= 0 ||
      (adjustedEnergyCost > 0 && this.energy < adjustedEnergyCost)
    ) {
      canFire = false;
    ...
    if (
      (input.keys.includes('KeyQ') || input.keys.includes('KeyE')) &&
      soundManager &&
      ...
      if (input.keys.includes('KeyQ')) {
        ...
        this.weaponIndex =
          (this.weaponIndex - 1 + this.weapons.length) % this.weapons.length;
      } else {
      ...
      if (
        previousWeapon === 'Mining Laser' &&
        this.currentWeapon !== 'Mining Laser'
      ) {
      ...
      if (window.shopSystem && window.shopSystem.availableWeapons) {
        const weapon = window.shopSystem.availableWeapons.find(
          (w) => w.id === this.currentWeaponId,
        );
      ...
      if (previousWeapon !== this.currentWeapon) {
        ...
        // Update weapon icon based on weapon type - match shop icons exactly
    ...
    const hasNumberKeyPressed = input.keys.some((key) =>
    ...
    if (soundManager && this.weaponSwitchCooldown <= 0) {
      ...
      if (selectedWeapon && this.weapons.includes(selectedWeapon)) {
        ...
        if (newIndex >= 0) {
          ...
          this.currentWeaponId =
            weaponNameToId[this.currentWeapon] || 'disengaged';
          ...
          if (window.shopSystem && window.shopSystem.availableWeapons) {
            ...
            if (weapon && weapon.stats) {
              ...
              this.fireCooldownTime =
                fallbackRates[this.currentWeaponId] || 0.12;
            }
          }
    ...
    if (
      ...
      for (let i = this.projectiles.length - 1; i >= 0; i--) {
        ...
        Object.values(window.game.multiplayer.players).forEach(
          (remotePlayer) => {
            ...
            if (distSq < collisionRadiusSq) {
              ...
              if (
                window.game.world &&
                !window.game.world.isInSafeZone(remotePlayer)
              ) {
                ...
                window.game.multiplayer.sendHit(
                  'player',
                  remotePlayer.id,
                  totalDamage,
                );
                ...
                if (
                  remotePlayer.shield <= 0 &&
                  remotePlayer.health <= projectile.damage
                ) {
                  ...
                  console.log(
                    '💀 Kill predicted by local client:',
                    this.name,
                    'vs',
                    remotePlayer.name,
                  );
      ...
      for (let i = this.projectiles.length - 1; i >= 0; i--) {
        ...
        if (
          this.projectiles[i].distanceTraveled > this.projectiles[i].range ||
          ...
          for (let i = remotePlayer.projectiles.length - 1; i >= 0; i--) {
            ...
            if (distSq < collisionRadiusSq) {
              ...
              if (!window.game.world || !window.game.world.isInSafeZone(this)) {
                console.log(
                  `💥 Remote projectile hit local player! Damage: ${projectile.damage}, Type: ${projectile.type}`,
                );
                ...
                if (
                  ...
                  console.log(
                    `💥 Creating explosion for ${projectile.type} with radius ${projectile.explosionRadius || 25}`,
                  );
                  if (window.game.world) {
                    ...
                    console.log(
                      `✅ Explosion created at (${projectile.x}, ${projectile.y})`,
                    );
                  } else {
                  ...
                  if (projectile.explosionDamage > 0) {
                    const totalDamage =
                      projectile.damage + projectile.explosionDamage;
    ...
    if (this.energy < this.maxEnergy) {
      ...
      this.energy = Math.min(
        this.maxEnergy,
        this.energy + energyRegenRate * deltaTime,
      );
    }
  ...
  checkMineCollisions() {
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      if (
        distSqLocal < mineProximityRadiusSq &&
        !window.game.world.isInSafeZone(this)
      ) {
        ...
        Object.values(window.game.multiplayer.players).forEach(
          (remotePlayer) => {
            ...
            if (
              ...
              // Show message
              window.game.multiplayer.showGameMessage(
                `Mine destroyed ${remotePlayer.name}!`,
                '#4f4',
              );
  ...
  die() {
    ...
    if (
      window.game &&
      window.game.multiplayer &&
      window.game.multiplayer.handleDeath
    ) {
      window.game.multiplayer.handleDeath(this.lastDamageFrom);
    }
  }
  ...
  fire(soundManager) {
    ...
    if (window.game && window.game.shop) {
      const weapon = window.game.shop.availableWeapons.find(
        (w) => w.name === this.currentWeapon,
      );
      if (weapon) {
    ...
    switch (this.currentWeapon) {
      ...
      case 'Burst Cannon':
        ...
        for (
          let i = -(burstCount > 1 ? Math.floor(burstCount / 2) : 0);
          i <= (burstCount > 1 ? Math.floor((burstCount - 1) / 2) : 0);
          i++
        ) {
          projectiles.push(
            new Projectile(
      ...
      case 'Space Mines':
        ...
        const mine = new Projectile(
          ...
          0,
          0, // No detonation ranges
          false,
          0, // No shield disruption
  ...
  render(ctx) {
    ...
    if (this.visible) {
      ...
      if (window.game && window.game.shipSkins) {
        ...
        window.game.shipSkins.renderShipWithSkin(
          ctx,
          tempShip,
        ...
        case 'heavy':
          if (this.thrustLevel > 0) {
            ...
            if (this.thrustLevel > 0.6) {
              // Left engine flicker
              const leftFlickerIntensity =
                Math.random() * 0.3 * this.thrustLevel;
              ctx.globalAlpha = 0.5 * leftFlickerIntensity;
              ctx.beginPath();
              ctx.arc(
                -10,
                22 + 6 * this.thrustLevel,
                4 * this.thrustLevel,
                0,
                Math.PI * 2,
              );
        ...
        case 'stealth':
          if (this.thrustLevel > 0) {
            ...
            const pulseIntensity =
              (Math.sin(Date.now() / 200) * 0.2 + 0.8) * this.thrustLevel;
            ctx.globalAlpha = 0.2 * pulseIntensity;
            ctx.beginPath();
            ctx.arc(
              0,
              14 + 4 * this.thrustLevel,
              4 * this.thrustLevel,
              0,
              Math.PI * 2,
            );
  ...
  createShieldHitEffect(hitX, hitY, playerX, playerY) {
    ...
    for (let i = 0; i < 2; i++) {
      // Create multiple ripples for a better effect
  ...
  renderRemoteProjectile(ctx, projectile) {
    ...
    if (
      projectile.glow ||
      ['plasma', 'quantum', 'missile'].includes(projectile.type)
    ) {
  ...
  updateMiningBeam(deltaTime, soundManager) {
    ...
    let weaponStats = null;
    if (window.shopSystem && window.shopSystem.availableWeapons) {
      weaponStats = window.shopSystem.availableWeapons.find(
        (w) => w.id === 'mininglaser',
      )?.stats;
    }
    ...
    if (
      ...
      for (const remotePlayer of Object.values(
        window.game.multiplayer.players,
      )) {
    ...
    if (hitTarget) {
      const now = Date.now() / 1000;
      if (now - this.miningBeam.lastDamageTime >= 0.1) {
        // 10 times per second
        ...
        if (now - this.miningBeam.lastSoundTime >= 0.3) {
          // Sound every 0.3 seconds
        ...
        if (hitTarget.type === 'asteroid') {
          ...
          const asteroidDamage =
            beamDamage * (weaponStats?.asteroidDamageMultiplier || 1.5); // Reduced from 3.0 to 1.5
          ...
          if (hitTarget.object.health <= 0) {
            ...
            if (window.game && window.game.achievements) {
              if (
                typeof window.game.achievements.onAsteroidDestroyed ===
                'function'
              ) {
            ...
            if (window.game && window.game.playerProfile) {
              if (
                typeof window.game.playerProfile.onCreditsEarned === 'function'
              ) {
                window.game.playerProfile.onCreditsEarned(creditReward);
              }
              if (
                typeof window.game.playerProfile.onAsteroidDestroyed ===
                'function'
              ) {
            ...
            if (window.game.world) {
              const asteroidIndex = window.game.world.asteroids.indexOf(
                hitTarget.object,
              );
              if (asteroidIndex !== -1) {
          ...
          if (
            window.game.world &&
            window.game.world.isInSafeZone(hitTarget.object)
          ) {
            // Skip damage for players in safe zone - no effects either
          } else {
            // Reduced damage to players
            const playerDamage =
              beamDamage * (weaponStats?.playerDamageMultiplier || 0.01);
  ...
  renderMiningBeam(ctx) {
    ...
    if (this.miningBeam.hitTarget) {
      ...
      gradient.addColorStop(
        0,
        `rgba(255, 100, 0, ${0.8 + this.miningBeam.intensity * 0.2})`,
      );
      gradient.addColorStop(
        0.5,
        `rgba(255, 150, 50, ${0.4 + this.miningBeam.intensity * 0.1})`,
      );
