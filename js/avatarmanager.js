...
export class AvatarManager {
  ...
  tryInitialize() {
    ...
    console.log(
      'Avatar manager initializing, found options:',
  ...
  setupPremiumAvatars() {
    ...
    for (
      let i = 0;
      i < premiumAvatarData.length && premiumSlotStart + i < allOptions.length;
      i++
    ) {
      ...
      console.log(
        `Setting up ${premiumData.name} (${premiumData.id}), owned: ${this.ownsAvatar(premiumData.id)}`,
      );
      ...
      if (this.ownsAvatar(premiumData.id)) {
        ...
        option.style.background =
          'linear-gradient(45deg, rgba(136, 136, 136, 0.1), rgba(0, 0, 0, 0.3))';
  ...
  setupAvatarSelection() {
    ...
    const modalAvatarOptions = document.querySelectorAll(
      '#avatarModal .avatar-option',
    );
    ...
    modalAvatarOptions.forEach((option) => {
      option.addEventListener('click', () => {
        ...
        if (
          this.premiumAvatars.includes(avatarType) &&
          !this.ownsAvatar(avatarType)
        ) {
  ...
  getAvatarDisplayName(avatarId) {
    ...
    if (this.premiumStore) {
      const premiumAvatar = this.premiumStore.premiumAvatars.find(
        (a) => a.id === avatarId,
      );
      if (premiumAvatar) {
  ...
  showPremiumHint(optionElement, avatarId) {
    try {
      ...
      hint.style.background =
        'linear-gradient(90deg, rgba(0,0,0,0.95), rgba(20,20,30,0.95))';
      ...
      hint.style.boxShadow =
        '0 6px 24px rgba(0,0,0,0.6), 0 2px 6px rgba(255,215,0,0.08)';
    ...
    try {
      ...
      if (btn) {
        ...
        if (!btn.dataset._premiumStyled) {
          ...
          btn.childNodes.forEach((node) => {
            if (
              ...
              /premium/i.test(node.textContent)
            ) {
              // Replace with span
              const replaced = document.createElement('span');
              replaced.innerHTML = node.textContent.replace(
                /(premium)/i,
                '<span class="premium-word" style="color:#ffd700;font-weight:700;">$1</span>',
              );
        ...
        setTimeout(() => {
          if (
            this.premiumStore &&
            typeof this.premiumStore.toggleStore === 'function'
          ) {
            try {
              this.premiumStore.toggleStore();
            } catch (e) {
              console.warn('toggleStore failed', e);
            }
            return;
          }
          ...
          if (
            window.premiumStore &&
            typeof window.premiumStore.toggleStore === 'function'
          ) {
            try {
              window.premiumStore.toggleStore();
            } catch (e) {
              console.warn('window.premiumStore.toggleStore failed', e);
            }
            return;
          }
        ...
        if (typeof window.open === 'function') {
          ...
          console.log(
            'Premium button not found; user hint shown but store not opened automatically.',
          );
        }
      }
    } catch (e) {
    ...
    try {
      ...
      const overlays = Array.from(
        document.querySelectorAll(
          '.modal-backdrop, .overlay, .dim-overlay, #modalBackdrop',
        ),
      );
      ...
      const premiumModals = premiumSelectors
      ...
      for (const modal of premiumModals) {
        ...
        premiumBackdrop = modal.querySelector(
          '.modal-backdrop, .overlay, .dim-overlay, #modalBackdrop',
        );
      ...
      overlays.forEach((el, idx) => {
        ...
        if (!el.dataset._oldOpacity)
          el.dataset._oldOpacity = el.style.opacity || '';
        if (!el.dataset._oldZ) el.dataset._oldZ = el.style.zIndex || '';
        if (!el.dataset._oldPointerEvents)
          el.dataset._oldPointerEvents = el.style.pointerEvents || '';
    ...
    try {
      ...
      dialogs.forEach((d) => {
        ...
        const shouldSkip = excludeSelectors.some((sel) => {
          try {
            return d.matches && d.matches(sel);
          } catch (e) {
            return false;
          }
        });
  ...
  openPremiumShopFlow() {
    try {
      ...
      if (shopModal) {
        ...
        this.premiumStore &&
        typeof this.premiumStore.openStore === 'function'
      ) {
        try {
          this.premiumStore.openStore();
        } catch (e) {
          console.warn('openStore failed', e);
        }
      } else if (
        window.premiumStore &&
        typeof window.premiumStore.openStore === 'function'
      ) {
        try {
          window.premiumStore.openStore();
        } catch (e) {
          console.warn('window.premiumStore.openStore failed', e);
        }
      } else {
      ...
      const observeAll = () => {
        const allModals = [
          ...Array.from(
            document.querySelectorAll('.modal, .panel, [role="dialog"]'),
          ),
          ...premiumSelectors
            .map((s) => document.querySelector(s))
            .filter(Boolean),
  ...
  restoreBackdropIfNoWindowsOpen() {
    try {
      ...
      const openWindows = Array.from(
        document.querySelectorAll(
          '.modal, .panel, [role="dialog"], .avatar-option',
        ),
      ).filter(
  ...
  drawAllAvatars() {
    // Draw modal avatars
    const modalAvatarCanvases = document.querySelectorAll(
      '#avatarModal .avatar-canvas',
    );
