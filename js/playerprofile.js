...
export class PlayerProfile {
  ...
  onHit(weapon) {
    ...
    this.stats.averageAccuracy =
      (this.stats.totalHits / this.stats.totalShots) * 100;
    this.saveStats();
  }
  ...
  onGameEnd() {
    if (this.currentGameStartTime) {
      ...
      this.stats.longestSurvival = Math.max(
  ...
  createProfileUI() {
    ...
    statsContainer.appendChild(
      this.createStatsSection('📊 General Statistics', [
        { label: 'Games Played', value: this.stats.gamesPlayed },
        {
          label: 'Total Play Time',
          value: this.formatTime(this.stats.totalPlayTime),
        },
        {
          label: 'Highest Score',
          value: this.stats.highestScore.toLocaleString(),
        },
        { label: 'Win Rate', value: this.getWinRate() + '%' },
        {
          label: 'Longest Survival',
          value: this.formatTime(this.stats.longestSurvival),
        },
      ]),
    );
    ...
    // Combat Stats
    statsContainer.appendChild(
      this.createStatsSection('⚔️ Combat Statistics', [
        {
          label: 'Total Shots Fired',
          value: this.stats.totalShots.toLocaleString(),
        },
        { label: 'Total Hits', value: this.stats.totalHits.toLocaleString() },
        {
          label: 'Average Accuracy',
          value: this.stats.averageAccuracy.toFixed(1) + '%',
        },
        ...
        {
          label: 'Asteroids Destroyed',
          value: this.stats.asteroidsDestroyed.toLocaleString(),
        },
        {
          label: 'Nemesis',
          value: this.stats.nemesis
            ? `${this.stats.nemesis} (${this.stats.nemesisKills} kills)`
            : 'None',
        },
      ]),
    );
    ...
    // Economic Stats
    statsContainer.appendChild(
      this.createStatsSection('💰 Economic Statistics', [
        {
          label: 'Current Credits',
          value: (this.player.credits || 0).toLocaleString(),
        },
        {
          label: 'Total Credits Earned',
          value: this.stats.totalCreditsEarned.toLocaleString(),
        },
        {
          label: 'Total Credits Spent',
          value: this.stats.totalCreditsSpent.toLocaleString(),
        },
        {
          label: 'Net Worth',
          value: (
            this.stats.totalCreditsEarned - this.stats.totalCreditsSpent
          ).toLocaleString(),
        },
      ]),
    );
    ...
    // Exploration Stats
    statsContainer.appendChild(
      this.createStatsSection('🌌 Exploration Statistics', [
        {
          label: 'Total Distance',
          value: this.formatDistance(this.stats.totalDistance),
        },
        {
          label: 'Average Speed',
          value: this.getAverageSpeed() + ' units/min',
        },
      ]),
    );
    ...
    if (Object.keys(this.stats.weaponStats).length > 0) {
      ...
      weaponStatsSection.innerHTML =
        '<h3 style="color: #4fc3f7; margin-bottom: 15px;">🗡️ Weapon Statistics</h3>';
    ...
    resetSection.innerHTML =
      '<h3 style="color: #ff6b6b; margin: 0 0 15px 0;">⚠️ Danger Zone</h3>';
    ...
    resetWarning.textContent =
      'This will permanently delete ALL your progress including stats, purchases, achievements, and settings. This cannot be undone!';
  ...
  getKDRatio() {
    if (this.stats.totalDeaths === 0) {
      return this.stats.totalKills > 0
  ...
  getAverageSpeed() {
    ...
    return Math.round(
      this.stats.totalDistance / (this.stats.totalPlayTime / 60),
    );
  }
