...
export class NPCManager {
  ...
  spawnDreadnaught() {
    ...
    const existingDreadnaught = this.npcs.find(
  ...
  update(deltaTime, player) {
    for (let i = this.npcs.length - 1; i >= 0; i--) {
      ...
      if (npc.type === 'alien_scout' && age > this.alienLifespan) {
        ...
        age > this.dreadnaughtRetreatTimer
      ) {
      ...
      if (
        !npc.isRemote &&
        (!npc.lastBroadcast || Date.now() - npc.lastBroadcast > 100)
      ) {
  ...
  updateAlienScout(alien, deltaTime, player) {
    ...
    if (alien.state === 'emerging') {
      ...
      const progress =
        alien.emergingEffect.elapsed / alien.emergingEffect.duration;
    ...
    if (alien.state === 'leaving') {
      ...
      alien.velocity.x =
        Math.cos(alien.rotation - Math.PI / 2) * alien.speed * deltaTime;
      alien.velocity.y =
        Math.sin(alien.rotation - Math.PI / 2) * alien.speed * deltaTime;
    } else if (
      closestTarget &&
      closestDistance < alien.aggroRange &&
      !this.isInSafeZone(closestTarget)
    ) {
      // Attack mode
      ...
      if (this.isInSafeZone(alien)) {
        ...
        alien.velocity.x =
          Math.cos(Math.atan2(escapeY - alien.y, escapeX - alien.x)) *
          alien.speed *
          deltaTime;
        alien.velocity.y =
          Math.sin(Math.atan2(escapeY - alien.y, escapeX - alien.x)) *
          alien.speed *
          deltaTime;
      } else {
        // Face target
      ...
      if (
        alien.lastTargetTime > 0 &&
        Date.now() - alien.lastTargetTime > 30000
      ) {
        // 30 seconds without target
        alien.state = 'leaving';
        return;
      }
      ...
      const targetX =
        alien.patrolCenter.x + Math.cos(alien.patrolAngle) * alien.patrolRadius;
      const targetY =
        alien.patrolCenter.y + Math.sin(alien.patrolAngle) * alien.patrolRadius;
      ...
      alien.rotation = Math.atan2(dy, dx) + Math.PI / 2;
      alien.velocity.x =
        Math.cos(alien.rotation - Math.PI / 2) * alien.speed * 0.3 * deltaTime;
      alien.velocity.y =
        Math.sin(alien.rotation - Math.PI / 2) * alien.speed * 0.3 * deltaTime;
    }
  ...
  updateDreadnaught(dreadnaught, deltaTime, player) {
    ...
    if (dreadnaught.target && closestTarget) {
      ...
      const timeSinceLastSwitch =
        Date.now() - (dreadnaught.lastTargetSwitch || 0);
      const currentTargetValid =
        !dreadnaught.target.destroyed && currentTargetDistance < 1500;

      const shouldSwitch =
        !currentTargetValid || // Current target invalid or too far
        (timeSinceLastSwitch > 15000 &&
          closestDistance < currentTargetDistance * 0.3); // 15+ seconds AND new target is 70% closer

      if (shouldSwitch) {
        dreadnaught.target = closestTarget;
        dreadnaught.lastTargetSwitch = Date.now();
        console.log(
          `Dreadnaught switched target after ${(timeSinceLastSwitch / 1000).toFixed(1)}s`,
        );
      }
    } else {
      dreadnaught.target = closestTarget;
    ...
    if (dreadnaught.state === 'leaving') {
      ...
      const bounds = this.world.boundaries;
      const targetX =
        dreadnaught.x > 0 ? bounds.right + 300 : bounds.left - 300;
      const targetY =
        dreadnaught.y > 0 ? bounds.bottom + 300 : bounds.top - 300;
      ...
      if (
        distance < 100 ||
        (closestTarget && closestDistance < dreadnaught.aggroRange)
      ) {
      ...
      if (closestTarget && closestDistance < dreadnaught.attackRange) {
        const targetInSafeZone =
          this.world.isInSafeZone && this.world.isInSafeZone(closestTarget);
        if (!targetInSafeZone) {
          // Do NOT rotate to face the target. Instead, briefly pause movement and bombard.
          dreadnaught.pauseUntil =
            Date.now() + (this.dreadnaughtBombardPauseApproach || 2000);
      ...
      const safeZone = this.world.safeZone;
      const distanceToSafeZone = Math.sqrt(
        (dreadnaught.x - safeZone.x) ** 2 + (dreadnaught.y - safeZone.y) ** 2,
      );
      ...
      if (closestTarget && closestDistance < dreadnaught.attackRange) {
        ...
        if (!targetInSafeZone) {
          ...
          dreadnaught.pauseUntil =
            Date.now() + (this.dreadnaughtBombardPauseCombat || 1500);
          this.fireDreadnaughtWeapons(dreadnaught, targets, deltaTime);
        }
      }
    }
    ...
    if (dreadnaught.state !== 'leaving') {
      const bounds = this.world.boundaries;
      dreadnaught.x = Math.max(
        bounds.left + 200,
        Math.min(bounds.right - 200, dreadnaught.x),
      );
      dreadnaught.y = Math.max(
        bounds.top + 200,
        Math.min(bounds.bottom - 200, dreadnaught.y),
      );
    }
  }
  ...
  fireDreadnaughtWeapons(dreadnaught, targets, deltaTime) {
    ...
    if (dreadnaught.isRemote) {
      console.warn(
        'Attempted to fire dreadnaught weapons for remote NPC, blocked!',
      );
      return;
    }
    ...
    if (
      dreadnaught.burstCount < dreadnaught.burstMax &&
      ...
      if (dreadnaught.weaponMounts && dreadnaught.weaponMounts.length > 0) {
        dreadnaught.weaponMounts.forEach((mount, index) => {
          if (currentTime - mount.lastFire > 200) {
            // Stagger weapon fire
            const mountWorldX =
              dreadnaught.x +
              Math.cos(dreadnaught.rotation - Math.PI / 2) * mount.x -
              Math.sin(dreadnaught.rotation - Math.PI / 2) * mount.y;
            const mountWorldY =
              dreadnaught.y +
              Math.sin(dreadnaught.rotation - Math.PI / 2) * mount.x +
              Math.cos(dreadnaught.rotation - Math.PI / 2) * mount.y;
            ...
            if (target && !target.destroyed) {
                  ...
                  update: function (deltaTime) {
                    ...
                    if (proj.range && proj.range > 0) {
                      const remaining = Math.max(
                        0,
                        1 - proj.distanceTraveled / proj.range,
                      );
                      this.life = remaining;
                    }
                  },
              ...
              try {
                const flash = {
                  ...
                  color:
                    npcBossProjectile.type === 'missile'
                      ? '#ffffff'
                      : '#ffdd66',
                  life: 1.0,
                  maxLife: 0.12,
              ...
              try {
                const tracer = {
                  ...
                  y: mountWorldY,
                  velocityX:
                    (npcBossProjectile.velocityX || Math.cos(angle) * 300) *
                    0.6,
                  velocityY:
                    (npcBossProjectile.velocityY || Math.sin(angle) * 300) *
                    0.6,
                  size: npcBossProjectile.type === 'missile' ? 2 : 3,
                  color:
                    npcBossProjectile.type === 'missile'
                      ? npcBossProjectile.__wrapped
                        ? npcBossProjectile.__wrapped.trailColor || '#ff8'
                        : '#ff8'
                      : npcBossProjectile.color || '#ffaa00',
                  life: 1.0,
              ...
              if (this.soundManager) {
                try {
                  if (npcBossProjectile.type === 'missile') {
                    this.soundManager.play('missile', {
                      volume: 0.9,
                      position: { x: mountWorldX, y: mountWorldY },
                    });
                  } else if (
                    npcBossProjectile.type === 'rocket' ||
                    npcBossProjectile.type === 'fusionMortar'
                  ) {
                    this.soundManager.play('mortar', {
                      volume: 0.95,
                      position: { x: mountWorldX, y: mountWorldY },
                    });
                  }
      ...
      if (dreadnaught.burstCount >= dreadnaught.burstMax) {
        dreadnaught.pauseUntil =
          Date.now() + (this.dreadnaughtVolleyReload || 900);
  ...
  renderDreadnaught(ctx, dreadnaught) {
    ...
    ctx.fillStyle =
      healthPercent > 0.5 ? 'rgba(255, 255, 0, 0.8)' : 'rgba(255, 100, 0, 0.8)';
  ...
  updateNPCProjectiles(deltaTime, player) {
    ...
    for (let i = this.world.npcProjectiles.length - 1; i >= 0; i--) {
      ...
      if (distance < 20) {
        ...
        this.world.createProjectileHitEffect(
          projectile.x,
          projectile.y,
          15,
          this.soundManager,
  ...
  showMessage(text, color = '#fff', duration = 3000, flash = false) {
    ...
    if (
      ...
      window.game &&
      window.game.multiplayer &&
      window.game.multiplayer.addScreenFlash
    ) {
  ...
  emergencyCleanup() {
    ...
    const dreadnaughtExists = this.npcs.some(
      (npc) => npc.type === 'dreadnaught',
    );
    ...
    console.log(
      `Emergency cleanup complete. ${this.npcs.length} NPCs remaining.`,
    );
  }
  ...
  emergencyCleanup() {
    ...
    this.showMessage(
      `Emergency cleanup: ${this.npcs.length} NPCs remaining`,
      '#ff6b6b',
      3000,
    );
  }
