export class ShopSystem {
    ...
    this.availableWeapons = [
      {
        ...
        name: 'Disengaged',
        description:
          'Weapons systems offline. Energy recharges 50% faster when weapons are disengaged.',
      ...
      },
      {
        ...
        name: 'Mining Laser',
        description:
          'Continuous beam weapon that excels against asteroids but has reduced damage against ships.',
        price: 12000,
      ...
      },
      {
        ...
        name: 'Space Mines',
        description:
          'Deploy proximity mines behind your ship. Mines arm after 2 seconds and explode on contact.',
        price: 18000,
      ...
      },
      {
        ...
        name: 'Fusion Mortar',
        description:
          'Fires explosive fusion shells that detonate at range with massive area damage.',
        price: 25000,
      ...
      },
      {
        ...
        name: 'Railgun',
        description:
          'Electromagnetic accelerator that fires high-velocity spikes with devastating penetration power.',
        price: 35000,
      ...
      },
      {
        ...
        description:
          'Advanced weapon that phases through obstacles and disables enemy shields.',
        price: 45000,
  ...
  showSafeZoneWarning(event, message) {
    try {
      ...
      warning.style.background =
        'linear-gradient(135deg, rgba(40,0,0,0.95), rgba(60,20,20,0.95))';
      ...
      warning.style.boxShadow =
        '0 6px 24px rgba(255,68,68,0.3), 0 2px 6px rgba(255,68,68,0.1)';
  ...
  createTab(name, tabId) {
    ...
    tab.style.textShadow =
      this.currentTab === tabId ? '0 0 6px #33f6' : '0 0 2px #33f3';
    tab.style.borderBottom =
      this.currentTab === tabId ? '2px solid #33f' : 'none';
    tab.onclick = () => {
      ...
      // Update tab styling
      document
        .querySelectorAll('#shop-container > div:nth-child(2) > div')
        .forEach((tab) => {
  ...
  updateShopContent() {
    ...
    document.getElementById('shop-credits').textContent =
      `Credits: ${this.player.credits || 0}`;
  ...
  updateChallengeTabBadge() {
    try {
      if (
        !this.tabElements ||
        !this.tabElements['challenges'] ||
        !window.game ||
        !window.game.challengeSystem
      )
        return;
      ...
      if (ship.owned) {
        ...
        button.onclick = (e) => {
          if (
            ...
            this.showSafeZoneWarning(
              e,
              'You must be docked in a safe zone to change ships.',
            );
            return;
          }
        ...
        button.onclick = (e) => {
          if (
            ...
            this.showSafeZoneWarning(
              e,
              'You must be docked in a safe zone to purchase ships.',
            );
            return;
          }
  ...
  renderWeaponsTab(container) {
    ...
    this.availableWeapons.forEach((weapon) => {
      ...
      weaponCard.style.border = weapon.owned
        ? '1px solid #3f3'
        : '1px solid #33f';
      ...
      if (weapon.owned) {
        ...
        button.onclick = (e) => {
          if (
            ...
            this.showSafeZoneWarning(
              e,
              'You must be docked in a safe zone to purchase weapons.',
            );
            return;
          }
    ...
    this.availableUpgrades.forEach((upgrade) => {
      ...
      for (let i = 0; i < upgrade.maxLevel; i++) {
        ...
        dot.style.backgroundColor =
          i < upgrade.level ? '#3f3' : 'rgba(100, 100, 100, 0.5)';
  ...
  renderChallengesTab(container) {
    ...
    if (window && window.console)
      console.log(
        'Rendering Challenges tab. completed:',
        challengeSystem.completed,
        'claimed:',
        challengeSystem.claimed,
        'notified:',
        challengeSystem.notified,
      );
  ...
  createChallengeCard(challenge, type, challengeSystem) {
    const completedList =
      challengeSystem &&
      challengeSystem.completed &&
      challengeSystem.completed[type]
        ? challengeSystem.completed[type]
        : [];
    const claimedList =
      challengeSystem &&
      challengeSystem.claimed &&
      challengeSystem.claimed[type]
        ? challengeSystem.claimed[type]
        : [];
    ...
    challengeCard.style.backgroundColor = isCompleted
      ? 'rgba(0, 60, 30, 0.5)'
      : 'rgba(0, 30, 60, 0.3)';
    challengeCard.style.borderRadius = '5px';
    challengeCard.style.border = isCompleted
      ? '1px solid #3f3'
      : '1px solid #33f';
  ...
  renderAppearanceTab(container) {
    ...
    const drawShipPreview = () => {
      ...
      if (window.game && window.game.shipSkins) {
        ...
        const tempPlayer = {
          ...
          shipSkin:
            selectedSkin && selectedSkin !== 'none' ? selectedSkin : null,
    ...
    const shipColors = ['#33f', '#4f4', '#f44', '#ff4', '#f4f', '#4ff'];
    ...
    shipColors.forEach((color) => {
      ...
      colorButton.style.border =
        color === (this.player.shipColor || '#33f')
          ? '2px solid white'
          : '1px solid #555';
    ...
    const ownedSkins = allOwnedSkins.filter((skinId) => {
      ...
      const skinData = window.game.premiumStore.shipSkins.find(
        (skin) => skin.id === skinId,
      );
    ...
    if (ownedSkins.length > 0) {
      ...
      const existingSkinButtons = document.querySelectorAll(
    ...
    const setBtnState = () => {
      ...
      effectsToggle.style.border = enabled
        ? '1px solid #3f8'
        : '1px solid #666';
    };
    ...
    effectsHint.textContent =
      'Toggle animated glows, fire, stealth flicker, etc.';
    ...
    engineColors.forEach((color) => {
      ...
      colorButton.style.backgroundColor = color;
      colorButton.style.border =
        color === (this.player.engineColor || '#f66')
          ? '2px solid white'
          : '1px solid #555';
      colorButton.style.borderRadius = '4px';
      colorButton.style.cursor = 'pointer';
  ...
  buyShip(shipId) {
    ...
    if (window.analytics) {
      ...
      window.analytics.trackShipUpgraded(
        oldShip,
        ship.name || shipId,
        ship.price,
      );
    ...
    if (
      ...
      typeof window.game.achievements.onShopPurchase === 'function'
    ) {
    ...
    if (
      window.game &&
      window.game.playerProfile &&
      typeof window.game.playerProfile.onShopPurchase === 'function'
    ) {
    ...
    if (
      ...
      try {
        window.game.multiplayer.socket.emit('playerShipChanged', {
          id:
            window.game.multiplayer.playerId ||
            window.game.multiplayer.playerId,
          ship: shipId,
          shipSkin: 'none',
        });
  ...
  buyWeapon(weaponId) {
    ...
    if (window.analytics) {
      window.analytics.trackWeaponPurchased(
        weapon.name || weaponId,
        weapon.price,
      );
      window.analytics.trackShopInteraction(
        'purchased',
        weapon.name || weaponId,
      );
    }
    ...
    switch (upgrade.id) {
      case 'engine':
        this.player.maxSpeed +=
          effects.maxSpeed -
          (upgrade.level > 1
            ? upgrade.getEffect(upgrade.level - 1).maxSpeed
            : 0);
        this.player.acceleration +=
          effects.acceleration -
          (upgrade.level > 1
            ? upgrade.getEffect(upgrade.level - 1).acceleration
            : 0);
        break;
      ...
      case 'energy':
        ...
        if (this.player.energy === undefined) {
          ...
          const increase =
            effects.maxEnergy -
            (upgrade.level > 1
              ? upgrade.getEffect(upgrade.level - 1).maxEnergy
              : 0);
          this.player.energy = Math.min(
            this.player.energy + increase,
            effects.maxEnergy,
          );
        }
        break;
      ...
      case 'armor':
        this.player.maxHealth +=
          effects.maxHealth -
          (upgrade.level > 1
            ? upgrade.getEffect(upgrade.level - 1).maxHealth
            : 0);
        this.player.damageReduction = effects.damageReduction;
