export class World {
  ...
  constructor() {
    ...
    console.log(
      'Multiplayer mode - waiting for server asteroids, NO local generation',
    );
  ...
  generateStarLayers() {
    ...
    for (let i = 0; i < this.starCount.far; i++) {
      ...
      const colorSet =
        Math.random() < 0.8 ? colorPalettes.white : colorPalettes.blue;
  ...
  setupServerAsteroidField(serverAsteroids) {
    console.log(
      'Setting up server asteroid field:',
      serverAsteroids.length,
      'asteroids',
    );
    ...
    console.log(
      ...
      'asteroids from server - all stationary',
    );
  }
  ...
  updateAsteroidHealth(asteroidId, damage) {
    ...
    if (asteroid) {
      ...
      console.log(
        `Updated asteroid ${asteroidId} health to ${asteroid.health} after ${damage} damage`,
      );
    }
  }
  ...
  destroyServerAsteroid(asteroidId, createExplosion = true) {
    ...
    if (asteroidIndex >= 0) {
      ...
      console.log(
        `Destroyed asteroid ${asteroidId} by server command. ${this.asteroids.length} asteroids remaining locally.`,
      );
    }
  }
  ...
  update(deltaTime, player, soundManager) {
        ...
        if (distance < asteroid.radius) {
          ...
          if (asteroid.health <= 0) {
            ...
            if (
              ...
              typeof window.game.achievements.onAsteroidDestroyed === 'function'
            ) {
    ...
    if (window.game && window.game.npcManager && window.game.npcManager.npcs) {
      for (
        let npcIndex = window.game.npcManager.npcs.length - 1;
        npcIndex >= 0;
        npcIndex--
      ) {
        ...
        for (
          let projectileIndex = player.projectiles.length - 1;
          projectileIndex >= 0;
          projectileIndex--
        ) {
          ...
          if (distance < hitRadius + 10) {
            ...
            console.log(
              `Hit NPC ${npc.type} for ${projectile.damage} damage. Health: ${npc.health}/${npc.maxHealth}`,
            );
    ...
    if (otherPlayers) {
      ...
      Object.values(otherPlayers).forEach((otherPlayer) => {
        ...
        if (distance < player.collisionRadius + 15) {
          // Using 15 as approximate collision radius for other players
    ...
    if (this.npcProjectiles && this.npcProjectiles.length > 0) {
      ...
      allPlayers.forEach((targetPlayer) => {
        if (!targetPlayer || targetPlayer.destroyed || targetPlayer.health <= 0)
          return;
        ...
        for (let i = this.npcProjectiles.length - 1; i >= 0; i--) {
          ...
          const collisionRadius =
            projectile.size +
            (targetPlayer.collisionRadius || targetPlayer.width / 2);
          ...
          if (distance < collisionRadius) {
            ...
            if (targetPlayer === player) {
              ...
              console.log(
                `NPC projectile hit local player for ${damage} damage!`,
              );
            } else {
              ...
              console.log(
                `NPC projectile hit remote player ${targetPlayer.id} for ${damage} damage!`,
              );
            }
  ...
  createProjectileHitEffect(x, y, radius, soundManager) {
    ...
    setTimeout(() => {
      if (this.explosions) {
        // Safety check in case World was destroyed
  ...
  applyPowerup(player, type) {
    ...
    switch (type) {
      case 'fireRate':
        ...
        player.fireCooldownTime =
          player.originalFireCooldownTime * fireRateBoost.multiplier;
      ...
      case 'shield':
        ...
        if (player.shieldCapacity === 0) {
          ...
          player.shield = Math.min(
            player.shield + player.shieldCapacity * 0.5,
            player.shieldCapacity,
          );
  ...
  updateFireRateBoosts(player) {
    ...
    if (!hasActiveBoosts) {
      ...
      if (player.originalFireCooldownTime) {
        player.fireCooldownTime =
          player.originalFireCooldownTime * strongestMultiplier;
      }
    }
  }
  ...
  render(ctx, player) {
    ...
    if (
      ...
      ctx.strokeText(
        'SAFE ZONE',
        this.safeZone.x,
        this.safeZone.y - this.safeZone.size / 2 - 20,
      );
      ctx.fillText(
        'SAFE ZONE',
        this.safeZone.x,
      ...
      ctx.translate(
        this.safeZone.x,
        this.safeZone.y - this.safeZone.size / 2 + 30,
      );
      ...
      ctx.save();
      ctx.translate(
        this.safeZone.x - this.safeZone.size / 2 + 30,
        this.safeZone.y,
      );
    ...
    this.asteroids.forEach((asteroid) => {
      ...
      if (!asteroid.vertices || !Array.isArray(asteroid.vertices)) {
        console.warn(
          `Asteroid ${asteroid.id} missing vertices, generating new ones`,
        );
        asteroid.vertices = this.generateAsteroidVertices(
          Math.floor(6 + Math.random() * 4),
          0.5,
        );
      }
    ...
    if (gameState === 'playing' || gameState === 'dying') {
      ...
      this.explosions.forEach((explosion) => {
        ...
        const opacity =
          explosion.timeLeft * (explosion.type === 'rocket' ? 1.4 : 2);
  ...
  renderStarLayers(ctx, player, visibleArea) {
    ...
    for (
      let layerIndex = 0;
      layerIndex < this.starLayers.length - 1;
      layerIndex++
    ) {
        ...
        if (star.type === 'bright') {
          ...
          if (star.hasGlow) {
            ...
            gradient.addColorStop(
              0.5,
              star.color.replace('80%', '50%').replace(')', ', 0.5)'),
            );
          ...
          gradient.addColorStop(
            0.5,
            star.color.replace('75%', '50%').replace(')', ', 0.3)'),
          );
          gradient.addColorStop(1, 'transparent');
    ...
    this.shootingStars.forEach((shootingStar) => {
      ...
      gradient.addColorStop(
        0.6,
        `rgba(${parseInt(colorBase.substr(0, 2), 16)}, ${parseInt(colorBase.substr(2, 2), 16)}, ${parseInt(colorBase.substr(4, 2), 16)}, 0.5)`,
      );
      ...
      const twinkleSize =
        shootingStar.size *
        3 *
        Math.abs(Math.sin(shootingStar.life * shootingStar.twinkleFreq));
      ctx.fillStyle = 'rgba(255, 255, 255, ' + shootingStar.life * 0.7 + ')';

      ctx.beginPath();
      ctx.moveTo(shootingStar.x, shootingStar.y - twinkleSize);
      ctx.lineTo(
        shootingStar.x + twinkleSize / 3,
        shootingStar.y - twinkleSize / 3,
      );
