...
export class MultiplayerManager {
  ...
  resetPlayerProgress(oldVersion) {
    ...
    try {
      ...
      console.warn('Could not clear disclaimer agreement:', e);
    }
    ...
    try {
      ...
      if (window.game && window.game.achievements) {
        try {
          ...
          if (
            ach.achievements &&
            typeof ach.achievements.forEach === 'function'
          ) {
            ...
            if (typeof ach.achievements.entries === 'function') {
              ...
              try {
                Object.values(ach.achievements).forEach((a) => {
                  a.progress = 0;
                  a.unlocked = false;
                });
              } catch (e) {}
    ...
    const preserveRegex =
      /(?:\bskin\b|\bavatar\b|premium|purchase|purch|purchased|owned|own|entitlement|shop|purchase_history|purchasedItems|ownedItems)/i;
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
  ...
  // accidental deletion of unrelated keys.
    ...
    try {
      ...
      const restoredHasSet =
        localStorage.getItem('hasSetName') === 'true' ||
        localStorage.getItem('has_set_name') === 'true';
      if (restoredHasSet) {
        this.hasSetName = true;
      }
      console.log(
        '💾 Restored in-memory playerName and hasSetName from preserved data',
      );
    } catch (e) {
      console.warn(
        'Could not restore in-memory player name/flags after reset:',
        e,
      );
    }
  ...
  #progress-reset-modal button { font-size: 14px !important; }
    ...
    console.log(
      `🔄 Progress reset complete! ${oldVersion} → ${this.GAME_VERSION}`,
    );
  }
  ...
  refreshGameStateAfterReset() {
    ...
    if (this.game && this.game.player) {
      ...
      this.game.player._credits = parseInt(
        localStorage.getItem('playerCredits') || 0,
      );
  ...
  ensureCreditsReset() {
    if (this.resetOccurred) {
      console.log(
        '🔄 Ensuring credits are reset in UI after reset occurred...',
      );
      const creditsElement = document.getElementById('credits');
      if (creditsElement) {
        creditsElement.textContent = '0';
        console.log(
          '💰 Credits display confirmed reset to 0 after UI creation',
        );
      }
    }
  }
  ...
  debugGameState() {
    ...
    if (this.game && this.game.player) {
      ...
      console.log(
        `  Credits: ${this.game.player._credits || this.game.player.credits}`,
      );
  ...
  connect(serverUrl = 'http://localhost:3000') {
    ...
    return connectionPromise.then(async () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        script.integrity =
          'sha384-/KNQL8Nu5gCHLqwqfQjA689Hhoqgi2S84SNUxC3roTe4EhJ9AfLkp8QiQcU8AMzI';
      ...
      confirmButton.onclick = () => {
        ...
        if (containsProfanity(name)) {
          feedback.textContent =
            'Name rejected by the Galactic Council. Try something cleaner!';
          input.focus();
          return;
        }
  ...
  setupSocketEvents() {
    ...
    this.socket.on('connect', () => {
      ...
      this.socket.emit('playerJoin', {
        ...
        shipSkin:
          this.game.player.shipSkin && this.game.player.shipSkin !== 'none'
            ? this.game.player.shipSkin
            : localStorage.getItem('selectedShipSkin') || 'none',
        skinEffectsEnabled:
      ...
      if (
        ...
        typeof this.game.playerProfile.onGameStart === 'function'
      ) {
    ...
    this.socket.on('playerData', (playerData) => {
      ...
      if (this.players[playerData.id]) {
        this.players[playerData.id].name =
          playerData.name || this.players[playerData.id].name;
        // Normalize ship property: keep both `ship` (legacy) and `currentShip` in sync
        const incomingShip =
          playerData.ship || this.players[playerData.id].ship;
        this.players[playerData.id].ship = incomingShip;
        this.players[playerData.id].currentShip =
          incomingShip || this.players[playerData.id].currentShip;
        this.players[playerData.id].color =
          playerData.color || this.players[playerData.id].color;
        this.players[playerData.id].avatar =
          playerData.avatar || this.players[playerData.id].avatar;
    ...
    this.socket.on('asteroidFieldUpdate', (data) => {
      console.log(
        'Received asteroid field from server:',
    ...
    this.socket.on('newAsteroids', (data) => {
      console.log(
        'Received new asteroids from server:',
        data.asteroids.length,
        'asteroids',
      );
      if (this.game.world) {
        ...
        const newClientAsteroids = data.asteroids.map((serverAsteroid) => {
          ...
          return {
            ...
            rotationSpeed:
              serverAsteroid.rotationSpeed || (Math.random() - 0.5) * 2,
            vertices: this.game.world.generateAsteroidVertices(
        ...
        console.log(
          'Added',
          newClientAsteroids.length,
          'new asteroids. Total:',
          this.game.world.asteroids.length,
        );
      }
    });
    ...
    this.socket.on('asteroidHit', (data) => {
      if (this.game.world) {
        ...
        const asteroid = this.game.world.asteroids.find(
          (a) => a.id === data.asteroidId,
        );
        if (asteroid) {
    ...
    // NEW: Handle periodic full snapshot from server (replacement of entire field)
    this.socket.on('worldSnapshot', (data) => {
      console.log('Received world snapshot from server');
      if (this.game.world && data.asteroids) {
        // Replace full asteroid array
        this.game.world.setupServerAsteroidField(data.asteroids);
      }
      if (this.game.world && data.powerups) {
        this.game.world.powerups = data.powerups;
      }
    });

    // NEW: Handle delta updates (created/updated/destroyed asteroids)
    this.socket.on('worldDelta', (data) => {
      if (!this.game.world) return;

      try {
        // Created asteroids - append to world
        if (data.asteroidsCreated && Array.isArray(data.asteroidsCreated)) {
          const newClientAsteroids = data.asteroidsCreated.map(a => ({
            id: a.id,
            x: a.x,
            y: a.y,
            rotation: a.rotation || 0,
            radius: a.radius,
            health: a.health || (a.radius <= 25 ? 50 : a.radius <= 45 ? 100 : 200),
            // keep other defaults consistent with setupServerAsteroidField
          }));
          this.game.world.asteroids.push(...newClientAsteroids);
        }

        // Updated asteroids - find and update fields
        if (data.asteroidsUpdated && Array.isArray(data.asteroidsUpdated)) {
          for (const u of data.asteroidsUpdated) {
            const idx = this.game.world.asteroids.findIndex(x => x.id === u.id);
            if (idx !== -1) {
              const asteroid = this.game.world.asteroids[idx];
              if (typeof u.x === 'number') asteroid.x = u.x;
              if (typeof u.y === 'number') asteroid.y = u.y;
              if (typeof u.rotation === 'number') asteroid.rotation = u.rotation;
              if (typeof u.health !== 'undefined') asteroid.health = u.health;
            }
          }
        }

        // Destroyed asteroids - remove by id
        if (data.asteroidsDestroyed && Array.isArray(data.asteroidsDestroyed)) {
          for (const id of data.asteroidsDestroyed) {
            const idx = this.game.world.asteroids.findIndex(x => x.id === id);
            if (idx !== -1) {
              // Remove the asteroid and let existing destruction handlers create fragments/effects if needed
              this.game.world.asteroids.splice(idx, 1);
            }
          }
        }

        // Powerups replacement if present
        if (data.powerups && Array.isArray(data.powerups)) {
          this.game.world.powerups = data.powerups;
        }
      } catch (e) {
        console.error('Error applying worldDelta:', e);
      }
    });
    ...
    this.socket.on('connect_error', (error) => {
      ...
      if (this.connectionErrorCount <= 3) {
        ...
        console.error(
          'Connection errors continuing... (suppressing further logs)',
        );
      }
    ...
    this.socket.on('gameState', (data) => {
      ...
      if (data.asteroids && this.game.world) {
        this.game.world.setupServerAsteroidField(data.asteroids);
        console.log(
          'Synchronized asteroid field from game state:',
          data.asteroids.length,
          'asteroids',
        );
      }
    ...
    this.socket.on('projectileHit', (data) => {
      ...
      if (data.targetId !== this.playerId) {
        this.handleRemoteProjectileHit(
          data.targetId,
          data.position,
          data.damage,
        );
    ...
    this.socket.on('playerDestroyed', (data) => {
      ...
      if (!killerName) {
        killerName =
          ...
          (data.attackerId === this.playerId
            ? 'You'
            : this.players[data.attackerId]?.name ||
              this.playerNameCache[data.attackerId] ||
              'Another player');
      }
      ...
      const victimName =
        data.playerId === this.playerId
          ? 'you'
          : this.players[data.playerId]?.name ||
            this.playerNameCache[data.playerId] ||
            'another player';
      ...
      if (data.attackerId === this.playerId) {
        ...
        if (this.ensureKillAnnouncerReady()) {
          this.killAnnouncer.announceKill(
            'You',
            victimName,
            'destroyed',
            killerAvatar,
        ...
        if (this.ensureKillAnnouncerReady()) {
          ...
          console.error(
            '❌ KillAnnouncer not ready for spectator announcement!',
          );
          // Fallback: show a game message
          this.showGameMessage(
    ...
    this.socket.on('playerLeft', (playerId) => {
      ...
      if (this.respawningPlayers.has(playerId)) {
        console.log(
          `Player ${playerId} left during respawn - not showing disconnect message`,
        );
    ...
    this.socket.on('playerNameChanged', (data) => {
      ...
      if (this.players[data.id]) {
        ...
        if (data.id !== this.playerId) {
          this.showGameMessage(
            `${data.oldName} is now known as ${data.newName}`,
            '#fff',
          );
        }
      }
    ...
    this.socket.on('playerTimedOut', (data) => {
      ...
      this.showGameMessage(
        `${data.name} was disconnected due to inactivity`,
        '#ff9',
      );
  ...
  ensureKillAnnouncerReady() {
    ...
    if (
      !this.killAnnouncer ||
      typeof this.killAnnouncer.announceKill !== 'function'
    ) {
  ...
  update(deltaTime) {
    ...
    if (this.lastUpdate >= this.updateInterval) {
      const playerData = {
        ...
        thrustLevel:
          typeof this.game.player.thrustLevel !== 'undefined'
            ? this.game.player.thrustLevel
            : 0,
    ...
    if (this.game && this.game.player && this.socket && this.connected) {
      ...
      const thrustDelta = Math.abs(
        (this.lastSentThrustLevel || 0) - currentThrust,
      );
      if (
        thrustDelta > 0.08 ||
        currentAfterburner !== this.lastSentAfterburner
      ) {
    ...
    if (document.getElementById('player-list-container')) {
      ...
      const leaderboardState = JSON.stringify(
        allPlayers.map((p) => ({
          id: p.id,
          score: p.score,
          wins: p.wins,
  ...
  broadcastStatsUpdate() {
    ...
    console.log('Force broadcasting player stats:', {
      score: playerData.score,
      wins: playerData.wins,
      losses: playerData.losses,
    });
  ...
  updateRemotePlayer(playerData) {
    ...
    if (player && !player.destroyed) {
      // Skip updates for destroyed players
      ...
      player.health =
        playerData.health !== undefined ? playerData.health : player.health;
      player.shield =
        playerData.shield !== undefined
          ? playerData.shield
          : player.shield || 0;
      player.maxShield =
        playerData.maxShield !== undefined
          ? playerData.maxShield
          : player.maxShield || 0;
      // Keep legacy `ship` and newer `currentShip` properties in sync
      const updatedShip =
        playerData.currentShip ||
        playerData.ship ||
        player.currentShip ||
        player.ship ||
        'scout';
      ...
      if (playerData.shipSkin !== undefined) {
        ...
        if (!player.currentShip || player.currentShip === 'scout') {
          ...
          player.currentShip =
            playerData.currentShip || playerData.ship || player.currentShip;
        }
      }
      ...
      player.score =
        playerData.score !== undefined ? playerData.score : player.score;
      player.wins =
        playerData.wins !== undefined ? playerData.wins : player.wins;
      player.losses =
        playerData.losses !== undefined ? playerData.losses : player.losses;
      ...
      if (
        ...
        console.log(
          `REMOTE PLAYER UPDATE: ${player.name} - Score: ${oldScore}->${player.score}, Wins: ${oldWins}->${player.wins}, Losses: ${oldLosses}->${player.losses}`,
        );
      }
      ...
      // If player has Unknown name but we have a cached name, restore it
        ...
        player.name === 'Unknown' &&
        this.playerNameCache[playerData.id]
      ) {
  ...
  handleRemoteProjectile(playerId, projectileData) {
    ...
    const projectile = {
      ...
      speed:
        ...
        (projectileData.type === 'missile' && projectileData.homing
          ? 350
          : 300), // Slower for homing missiles
      ...
      update: function (deltaTime, world) {
        ...
        if (this.homing && this.type === 'missile' && world) {
          ...
          if (
            !this.hasHomed &&
            this.distanceTraveled >= (this.homingMinDistance || 200)
          ) {
            ...
            if (target && targetDist <= homingRadius) {
              ...
              if (Math.abs(angleDiff) < 0.5) {
                // When reasonably close to target angle (was 0.3)
                this.hasHomed = true;
              }
            }
          }
      ...
      render: function (ctx) {
        ...
        if (
          window.game &&
          window.game.player &&
          window.game.player.renderRemoteProjectile
        ) {
    ...
    if (this.players[playerId] && !this.players[playerId].destroyed) {
      // Skip for destroyed players
  ...
  handleDamage(damage, attackerId) {
    ...
    if (this.game.world) {
      ...
      setTimeout(() => {
        ...
        const secondaryX =
          this.game.player.x + Math.cos(secondaryAngle) * secondaryOffset;
        const secondaryY =
          this.game.player.y + Math.sin(secondaryAngle) * secondaryOffset;
  ...
  handleShieldDisruption(duration, attackerId) {
    ...
    setTimeout(() => {
      if (
        this.game.player &&
        this.game.player.shieldDisrupted &&
  ...
  handleDeath(attackerId) {
    ...
    if (
      this.game.playerProfile &&
      typeof this.game.playerProfile.onGameEnd === 'function'
    ) {
  ...
  handleRemotePlayerDeath(playerId, attackerId) {
    ...
    if (attackerId === this.playerId) {
      this.showGameMessage(
        `You destroyed ${deadPlayerName}! (+500 pts, +250 credits)`,
        '#4f4',
      );
      ...
      if (this.game.player) {
        ...
        if (
          this.game.achievements &&
          typeof this.game.achievements.onPlayerKill === 'function'
        ) {
          this.game.achievements.onPlayerKill(playerId);
        }
        if (
          this.game.achievements &&
          typeof this.game.achievements.onWin === 'function'
        ) {
          this.game.achievements.onWin();
        }
        if (
          this.game.playerProfile &&
          typeof this.game.playerProfile.onKill === 'function'
        ) {
          this.game.playerProfile.onKill(this.game.player.currentWeapon, true);
        }
        if (
          this.game.playerProfile &&
          typeof this.game.playerProfile.onWin === 'function'
        ) {
          this.game.playerProfile.onWin();
        }
        if (
          this.game.playerProfile &&
          typeof this.game.playerProfile.onCreditsEarned === 'function'
        ) {
  ...
  createRemoteShipDestructionEffect(deadPlayer) {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const colors = [
        playerColor,
        this.getLighterColor(playerColor),
        '#f66',
        '#ddd',
      ];
  ...
  handleRemotePlayerRespawn(playerId, x, y) {
    ...
    if (lastProcessed && now - lastProcessed < 1000) {
      ...
      console.log(
        'Preventing duplicate respawn processing for player:',
        playerId,
      );
      return;
    }
    ...
    if (this.players[playerId]) {
      ...
      if (this.playerNameCache[playerId]) {
        this.showGameMessage(
          `${this.playerNameCache[playerId]} respawned!`,
          '#8af',
        );
      } else {
      ...
      if (this.game.world.asteroids[i].id === data.asteroidId) {
        ...
        if (data.destroyedBy === this.socket.id && this.game.player) {
          ...
          if (asteroid.size !== 'small') {
            if (Math.random() < 0.3) {
              // 30% chance for large/medium asteroids
              this.game.world.spawnPowerup(asteroid.x, asteroid.y);
            }
          } else if (Math.random() < 0.1) {
            // 10% chance for small asteroids
            this.game.world.spawnPowerup(asteroid.x, asteroid.y);
          }
        }
  ...
  handleOtherPlayerAsteroidDestruction(data) {
    ...
    const asteroidIndex = this.game.world.asteroids.findIndex(
      (a) => a.id === data.asteroidId,
    );
    if (asteroidIndex >= 0) {
  ...
  updatePlayerList() {
    ...
    const remotePlayers = Object.values(this.players)
      .filter((p) => !p.destroyed)
      .map((p) => ({
        id: p.id,
        name: p.name,
        ...
        skinEffectsEnabled:
          p.skinEffectsEnabled === undefined ? true : p.skinEffectsEnabled,
        isSelf: false,
      }));
    ...
    const filteredRemotePlayers = remotePlayers.filter(
    ...
    allPlayers.forEach((player, index) => {
      ...
      if (window.ui && window.ui.avatarManager) {
        const avatarDataUrl = window.ui.avatarManager.generateSmallAvatar(
          player.avatar,
        );
        if (avatarDataUrl) {
    ...
    Object.values(this.players).forEach((player) => {
      ...
      if (player.shield && player.shield > 0) {
        ...
        const drawRipple = (phase, width, opacity) => {
          ...
          if (adjustedPhase < 0.7) {
            // Only show ripple during part of the cycle
        ...
        if (intensity > 0) {
          ...
          for (let i = 0; i < numBolts; i++) {
            ...
            for (let j = 1; j <= segments; j++) {
              const t = j / segments;
              const jaggedX =
                startX + (endX - startX) * t + (Math.random() - 0.5) * 8;
              const jaggedY =
                startY + (endY - startY) * t + (Math.random() - 0.5) * 8;
      ...
      if (player.miningBeam && player.miningBeam.active) {
        ...
        const gradient = ctx.createRadialGradient(
          player.miningBeam.targetX,
          player.miningBeam.targetY,
          0,
          player.miningBeam.targetX,
          player.miningBeam.targetY,
          glowSize,
        ...
        gradient.addColorStop(
          0,
          `rgba(255, 100, 0, ${0.8 + player.miningBeam.intensity * 0.2})`,
        );
        gradient.addColorStop(
          0.5,
          `rgba(255, 150, 50, ${0.4 + player.miningBeam.intensity * 0.1})`,
        );
      ...
      let usedShipSkin = false;
      if (
        player.shipSkin &&
        player.shipSkin !== 'none' &&
        this.game.shipSkins
      ) {
        ...
        this.game.shipSkins.renderShipWithSkin(
          ctx,
          tempPlayer,
          this.game.premiumStore,
        );
        usedShipSkin = true;
      }

      // Get engine color and thrust level for dynamic engine flames (needed for all ships)
      ...
      // Only draw basic ship geometry if no ship skin was used
      if (!usedShipSkin) {
        ...
        if (remoteShipType === 'fighter') {
          ...
          ctx.lineTo(4, -8);
          ctx.stroke();
        } // End of else (scout ship)
      } // End of if (!usedShipSkin)

      // --- Always render dynamic engine flames for all ships (whether using skins or not) ---
      // This ensures engine flames show for remote players just like for the local player
      switch (remoteShipType) {
        case 'fighter':
          if (thrustLevel > 0) {
            ...
            const fighterEngineGradient = ctx.createRadialGradient(
              0,
              12,
              0,
              0,
        ...
        case 'stealth':
          if (thrustLevel > 0) {
            ...
            const pulseIntensity =
              (Math.sin(Date.now() / 200) * 0.2 + 0.8) * thrustLevel;
        ...
        default: // scout and other ship types
          if (thrustLevel > 0) {
            ctx.fillStyle = engineColor;
            ctx.beginPath();
            ctx.moveTo(-5, 5);
  ...
  createLocalExplosionEffect(x, y, rotation) {
    ...
    for (let i = 0; i < 5; i++) {
      setTimeout(
        () => {
          if (!this.game || !this.game.world || this.game.gameState !== 'dying')
            return;
  ...
  sendPositionUpdate() {
    if (this.connected && this.game.player) {
      ...
      const currentSpeed = Math.sqrt(
        this.game.player.velocity.x ** 2 + this.game.player.velocity.y ** 2,
      );
      const thrustLevel = Math.min(
        1.0,
        currentSpeed / (this.game.player.maxSpeed * 0.7),
      );
  ...
  updateDreadnaughtScheduleDisplay(data) {
    ...
    if (data.dreadnaughtActive) {
      status =
        '🛸 <strong style="color: #ff4444;">DREADNAUGHT CURRENTLY ACTIVE</strong>';
    } else if (timeUntil <= 0) {
      status =
        '⚡ <strong style="color: #ffaa00;">READY TO SPAWN</strong> (waiting for conditions)';
    } else {
